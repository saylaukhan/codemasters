...
| **Симулятор против живого сервера (T-55)** — гонялся только `--dry-run` | карта, аналитика и инциденты на его данных не открывались | `make api` + `make simulate`, затем открыть панель |
...
Отдельно: `make check-backend` и `make check-web` в этой ветке не прогонялись — не было зависимостей
проекта и Docker. Прошли `make check-agent` целиком, `ruff check`, `ruff format --check` и
`pytest simulator/` (43 проверки). Первым делом на живой машине стоит прогнать `make check`.

Ещё одно ограничение стоит знать заранее: симулятор заливает **30 суток** истории, а не 90 —
сервер отклоняет замер старше срока очереди агента (31 сутки, ADR-006). «3 месяца истории» из
критерия T-55 через API агента недостижимы; нужно решение лида.
