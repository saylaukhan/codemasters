...
A rule watches one metric on every line: N violations in a row or T minutes of violation open
an incident, M normal results in a row set ``restored_at`` (ТЗ п. 18). The detection of T-40
reads the rules anew on every run, so an edit in the admin panel applies to the next one. A rule
is switched off with ``is_active``, never deleted: its incidents refer to it.
"""

from sqlalchemy import CheckConstraint, Identity, text
