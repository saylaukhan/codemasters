...
export const ASSISTANT_LABELS = {
  ...
  welcome:
    'Спросите, как пользоваться панелью: где найти раздел, что означает статус, как отправить обращение.',
  suggestions: 'Частые вопросы на этом экране',
  placeholder: 'Ваш вопрос о панели',
  ask: 'Спросить',
  restart: 'Начать заново',
  you: 'Вы',
  assistant: 'Помощник',
  thinking: 'Помощник пишет ответ',
  disclaimer: 'Помощник отвечает по описанию панели и не видит данные школ. Важное проверяйте в самой панели.',
} as const
...
export const ASSISTANT_SUGGESTIONS: Record<AssistantScreen, readonly string[]> = {
  overview: [
    'Что означает полоса статусов?',
    'Как открыть школы со статусом «Критично»?',
    'Что попадает в «Требуют внимания»?',
  ],
  map: ['Как найти школу на карте?', 'Что означают цвета точек на карте?', 'Как отфильтровать школы по району?'],
  schools: ['Как найти школу по коду?', 'Что означает статус «Нет данных»?', 'Как открыть карточку школы?'],
  school_card: [
    'Как создать обращение поставщику?',
    'Где посмотреть договор и линии школы?',
    'Как добавить ответственного?',
  ],
  school_cabinet: [
    'Что делать, если интернет медленный?',
    'Почему написано «Компьютер с агентом выключен»?',
    'Как сообщить о проблеме поставщику?',
    'Что означают «норма» и «договор» на плитках?',
  ],
  device_card: ['Что означает «Не оценивает линию»?', 'Почему компьютер заблокирован?', 'Как посмотреть замеры за 30 дней?'],
  incidents: ['Как открывается инцидент?', 'Чем «Список» отличается от «Доски»?', 'Как найти инцидент по номеру?'],
  incident_card: [
    'Как перевести инцидент в другой статус?',
    'Как создать обращение по инциденту?',
    'Что означают показатели-основания?',
  ],
  appeals: ['Где черновики обращений?', 'Что значит «Не отправлено»?', 'Как создать новое обращение?'],
  appeal_draft: ['Можно ли изменить текст письма?', 'Что делает «Перегенерировать»?', 'Что происходит после отправки?'],
  appeal_card: ['Как скачать PDF обращения?', 'Кто может закрыть обращение?', 'Как добавить комментарий?'],
  providers: ['Как считается оценка поставщика?', 'Что такое «Договор ниже норматива»?', 'Как скачать акт?'],
  rollout: ['Что значит «Молчат»?', 'Как назначить обновление агента?', 'Как выдать код установки?'],
  analytics: ['Как сравнить школы?', 'Что показывает тепловая карта?', 'Как выбрать свой период?'],
  exports: ['Как выгрузить замеры в XLSX?', 'Чем агрегаты отличаются от сырых данных?', 'Где скачать готовую выгрузку?'],
